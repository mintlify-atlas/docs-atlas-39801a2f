...
description: "Complete reference for LWXGL window creation, rendering, the event loop, modal dialogs, and resource cleanup functions."
---
...
Frees all registered elements, releases the graphics context, destroys the back-buffer pixmap and stipple bitmap, frees all 16 allocated palette colors, destroys the window, and closes the display connection. This function must be called after the main loop exits to avoid resource leaks.
...
<ParamField path="on_every" type="void (*)(int tick)" required>
